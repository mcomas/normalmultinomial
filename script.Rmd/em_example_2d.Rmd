...
# step = 0.1
moments_grid = function(x, m, inv_Sigma, step=0.1){
  p = log(x[1:2]/x[3])
  
  Ap = as.matrix(expand.grid( seq(p[1]-5, p[1]+5, 0.1), seq(p[2]-5, p[2]+5, 0.1) ))
  lik = exp(apply(Ap, 1, mvf, m, inv_Sigma, x))
  
  res_grid = list('mean' = c(0,0), 'Sigma' = matrix(0, nrow=2, ncol=2))
  for(i in 1:2){
    res_grid[['mean']][i] = sum(Ap[,i] * lik * step * step) / sum(lik * step * step)
    for(j in 1:2){
      res_grid[['Sigma']][i,j] = sum(Ap[,i] * Ap[,j] * lik * step * step) / sum(lik * step * step)
    }
  }
  res_grid
}

mom = t( apply(Xt, 1, moments_grid, m_t0, solve(Sigma_t0), step = 0.1) )
...
for(i in 1:nsteps){
  mom = t( apply(Xt, 1, moments_grid, m_t, solve(Sigma_t), step = 0.1) )
