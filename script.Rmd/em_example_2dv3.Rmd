...
# sigma = Sigma_t0
# nsim = 100
moments_rnd = function(x, m, sigma, nsim=100){
  p = log(x[1:2]/x[3])
  Ap = rnormal(n = nsim, mu = m, sigma = sigma)
  
  lik = exp(apply(Ap, 1, mvf_mult, x))
...
mom = apply(Xt, 1, moments_rnd, m_t0, Sigma_t0, nsim = 1000)
...
for(i in 1:nsteps){
  mom = t( apply(Xt, 1, moments_rnd, m_t, Sigma_t, nsim = 1000) )
