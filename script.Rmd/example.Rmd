...
library(dplyr)
d <- read.csv("~/research/packages/normalmultinomial/data/ContZero2Marc.csv", sep=';', stringsAsFactors=FALSE)
```
...
X = split(X, X$cluster)
cc = lapply(X, function(.data) normalmultinomial_fitting(as.matrix(.data[,-4])))
...
p = data.frame(
  'A' = sapply(1:nrow(X), function(i) logLike(a = as.numeric(alr(A[i,])), mu = muA, inv_sigma = inv_sigmaA, x = as.numeric(X[i,]))),
