...
df$fnorm1 = sapply(df$a, function(a) dnorm(a, mean = m, sd = sqrt(v)))
df$fnorm2 = sapply(df$a, function(a) dnorm(a, mean = m, sd = sqrt(SIGMA)))
points(df[,c('a','fnorm1')], type = 'l', col = 'red')
points(df[,c('a','fnorm2')], type = 'l', col = 'green')
abline(v = m, col = 'red')
...
x1 = replicate(1000,expectedA1(x = x, mu = MU, sigma = SIGMA, nsim=1000)[[1]])
x2 = replicate(1000,expectedA2(x = x, mu = MU, sigma = SIGMA, nsim=1000)[[1]])
x3 = replicate(1000,expectedA3(x = x, mu = MU, sigma = SIGMA, nsim=1000)[[1]])
